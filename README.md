# 比赛信息监控

这是一个自动抓取并展示 **智能体、电池、AI、储能、寿命预测** 相关比赛信息的在线网站，数据每日自动更新，无需手动运行代码，打开网页即可查看最新比赛信息。


## 网址链接

🔗 https://competitionmonitor.streamlit.app

> 部署在 [Streamlit Cloud](https://streamlit.io/cloud) 上

网站界面截图示例：
<img width="2240" height="1199" alt="image" src="https://github.com/user-attachments/assets/3de9b65d-5daa-4939-ac3f-8aa795c69e4c" />


## 功能特点

- **自动抓取**：每日自动从百度千帆搜索、指定公告页、指定 URL 抓取比赛信息
- **智能筛选**：通过 LLM 判断是否为真实比赛，自动过滤获奖名单、论坛、培训等无关内容
- **截止日期识别**：自动提取报名截止日期，已过期的自动丢弃，未明确的标注提醒
- **历史累积**：不会覆盖过往抓取信息，新发现的比赛自动追加到已有数据库
- **零成本部署**：GitHub Actions 定时任务 + Streamlit Cloud 免费托管，无需服务器


## 关键词筛选

当前监控以下 5 个关键词的比赛信息：

- 智能体
- 电池
- AI
- 储能
- 寿命预测

系统只保留包含**比赛/大赛/竞赛/挑战赛**字样，且带有**报名/参赛/作品提交**等语义的真实赛事。


## 数据来源

| 来源 | 说明 |
|------|------|
| `qianfan_search` | 百度千帆 AI 搜索，自动生成关键词组合查询 |
| `seed_pages` | 读取 `sources_seed_pages.txt` 中的公告列表页 |
| `seed_urls` | 读取 `sources_seed_urls.txt` 中的单篇 URL |

搜索策略：每轮执行单关键词和双关键词组合搜索，覆盖 5 个单关键词 + 10 个双关键词组合，共 15 个查询。


## 自动更新机制

项目使用 **GitHub Actions** 每天自动运行两次：

| 时间（北京时间） | 说明 |
|----------------|------|
| 08:00 | 早间自动抓取 |
| 20:00 | 晚间自动抓取 |

以保证信息更新及时。


## 部署说明

本项目采用 **零成本自动化部署** 架构：

1. **代码托管**：GitHub（免费）
2. **定时运行**：GitHub Actions（免费额度足够每日两次运行）
3. **网站托管**：Streamlit Cloud（免费，自动同步 GitHub 仓库）

如需重新部署：

1. Fork 本仓库
2. 在仓库 Settings → Secrets 中配置你的 API 密钥
3. 在 [Streamlit Cloud](https://streamlit.io/cloud) 导入仓库，主文件路径填 `streamlit.py`
4. 网站自动上线，数据每日自动更新


## 免责声明

本项目为**个人学习研究用途**的爬虫程序，仅供技术交流与学习参考，数据通过百度千帆 AI Search API 与智谱 GLM API 合法获取。

**使用请注意：**

1. 请遵守百度千帆与智谱平台的服务条款，不要将 API Key 用于超出授权范围的高频调用或转售。
2. 本项目 API Key 通过 GitHub Secrets 存储，请勿通过任何方式恶意泄露或传播开发者密钥。
3. 展示的比赛信息版权归原作者或原发布平台，本站仅做聚合展示，如有侵权请联系删除。
4. AI 提取的截止日期、奖项等信息可能存在偏差，报名请以官方原文为准，本页面信息仅供参考。

> 本项目仅供学习交流，作者不对任何后果承担责任。
